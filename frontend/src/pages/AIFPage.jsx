    ...
    <div>
      <section className="grad-hero border-b border-[#E6E8F0]">
        <div className="container-x py-16">
          ...
          <p className="mt-4 text-lg text-[#526071] max-w-2xl">Access professionally-managed AIFs across categories. Register your interest and our team will guide you through eligibility and onboarding.</p>
          <span className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#FEF3C7] text-[#9A4A05] px-3 py-1 text-xs font-semibold">Phase 2 — full flow coming soon</span>
        </div>
      </section>
      ...
      <section className="container-x py-16 grid lg:grid-cols-3 gap-6">
        {[
          ...
          <div key={c.t} className="surface p-7">
            ...
            <p className="mt-2 text-[15px] text-[#526071]">{c.d}</p>
          </div>
        ))}
      </section>
      ...
      <section className="container-x pb-20">
        <div className="surface p-8 lg:p-10 grid lg:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <p className="mt-2 text-[#526071]">Leave your email and our AIF desk will reach out with suitable options.</p>
            <ul className="mt-5 space-y-2 text-sm text-[#334155]">
              {['Minimum investment typically ₹1 Cr','Suitable for accredited investors','Dedicated relationship manager'].map(t => (
                <li key={t} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#0B7F4A]" /> {t}</li>
              ))}
            </ul>
          </div>
