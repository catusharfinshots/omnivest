...
export default function VersionDiff({ p }) {
  ...
  if (!isRebalance) {
    return (
      <div className="mt-1.5 inline-flex items-center gap-1 text-[12px] text-[#6B6480]" data-testid="launch-info">
  ...
  return (
    <div className="mt-2 rounded-lg border border-[#FDE68A] bg-[#FFFBEB] p-2.5 text-[12px]" data-testid="rebalance-diff">
      ...
      <div className="mt-1.5 grid sm:grid-cols-2 gap-x-4 gap-y-0.5 text-[#4B4560]">
        {rows.map((r) => (
          <div key={r.s} className="flex items-center gap-1.5">
            ...
            {r.kind === 'added' && <span className="text-[#0B7F4A]">added at {r.to}%</span>}
            {r.kind === 'removed' && <span className="text-[#B91C1C]">removed (was {r.from}%)</span>}
            {r.kind === 'changed' && <span className="inline-flex items-center gap-1">{r.from}% <ArrowRight className="h-3 w-3 text-[#667085]" /> <b className={r.to > r.from ? 'text-[#0B7F4A]' : 'text-[#B91C1C]'}>{r.to}%</b></span>}
          </div>
        ))}
