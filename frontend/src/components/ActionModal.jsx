          ...
          {preview && !b && (
            <>
              {isFix
                ...
                : <Note tone="warn" icon={AlertTriangle}>This sells <b>everything</b> this portfolio holds in your broker account, {preview.count} stock{preview.count > 1 ? 's' : ''}, at today's price minus {preview.buffer_pct}%. Shares bought today may not be sellable until they settle.</Note>}
              ...
              {funds ? (
                <div className="rounded-2xl border border-[#F1D48A] bg-[#FFFBEB] p-4" data-testid="action-funds-gate">
                  ...
                  <div className="mt-3 text-[13px] space-y-1.5">
                    ...
                    <div className="flex justify-between"><span className="text-[#526071]">Available at your broker</span><b className="num">{INR(funds.available)}</b></div>
              ...
              {!isFix && confirmExit && <Note tone="neg" icon={AlertTriangle}>Tap again to confirm. This cannot be undone once your broker executes the sells.</Note>}
              <div className="text-[11.5px] text-[#667085] text-center leading-relaxed">By placing, you instruct your broker to {preview.side === 'BUY' ? 'buy' : 'sell'} these quantities in your account. Execution prices may differ from the limits shown.</div>
            </>
          )}
          ...
          {b && (() => { const none = b.counts.placed === 0; const partial = !none && b.counts.rejected > 0; return (
            <div className="space-y-3" data-testid="action-placed">
              ...
              {none ? <Note tone="neg" icon={AlertTriangle}>Nothing was placed and nothing was charged. See the reason on the Orders page and try again.</Note>
                : b.mode === 'amo' ? <Note tone="pos" icon={CheckCircle2}>They execute when NSE opens on {when(result.market.next_open_ist)}. This page updates itself once your broker reports the fills.</Note>
                : <Note tone="pos" icon={CheckCircle2}>Your broker is executing them now. Check holdings in a minute to see them update.</Note>}
