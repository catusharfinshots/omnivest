...
const base = 'flex flex-col items-center justify-center gap-0.5 h-full text-[11px] font-medium transition-colors';
const active = 'text-[#6C2BD9]';
const idle = 'text-[#64748B] hover:text-[#1A1030]';
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button data-testid="mobtab-account" className={`${base} ${idle}`}>
              {isAuthed ? (
                <span className="h-5 w-5 rounded-full grad-card text-white grid place-items-center text-[10px] font-bold">
          ...
          <PopoverContent side="top" align="end" sideOffset={10} className="w-56 p-2 rounded-xl border-[#E6E8F0] mr-2">
            {isAuthed ? (
              <>
                ...
                <button onClick={doLogout} data-testid="mobnav-logout" className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-[#DC2626] hover:bg-[#FEF2F2]"><LogOut className="h-4 w-4" /> Log out</button>
              </>
            ) : (
