...
PRIVACY_HTML = (
    ...
    "<tr><td>Mobile number, one-time codes, name, email (optional)</td><td>Creating and signing in to your account</td><td>To identify you, secure your account and send you service messages</td></tr>"
    "<tr><td>PAN, name as on PAN, date of birth, state</td><td>Before your first paid subscription</td><td>To bill you correctly, confirm you are an adult and issue tax-compliant invoices</td></tr>"
    "<tr><td>Terms you signed: version, time, phone number, code confirmation, IP address</td><td>Signing a portfolio's terms</td><td>To keep proof of what you agreed to, as SEBI requires of partners</td></tr>"
    "<tr><td>Order and payment references, amount, plan, payment status</td><td>Paying for a subscription</td><td>To grant access, reconcile payments and handle disputes</td></tr>"
    "<tr><td>Watchlist, subscriptions, pages viewed, device and browser type, approximate location from IP</td><td>Using the Platform</td><td>To run and improve the product, fix errors and prevent abuse</td></tr>"
    "<tr><td>Broker identifier and the data your broker returns for actions you take</td><td>Only if you connect a broker</td><td>To show your holdings and place the orders you request</td></tr>"
    "<tr><td>Registration certificates, PAN, address, officer details</td><td>Applying as a partner</td><td>To verify SEBI registration and print licence-holder details for investors</td></tr>"
    "</tbody></table>"
...
def fill(html: str, d: Dict[str, str]) -> str:
    """Render {{#key}}…{{/key}} blocks only when the detail is set, then substitute {{key}} tokens."""
